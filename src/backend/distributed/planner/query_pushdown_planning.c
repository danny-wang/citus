...
{
	DeferredErrorMessage *message = DeferErrorIfCannotPushdownSubquery(subqueryTree, outerMostQueryHasLimit);
	if (IsLoggableLevel(DEBUG3))
	{	
		if (message != NULL) {
			ereport(DEBUG3, (errmsg("DeferErrorIfCannotPushdownSubquery, detail:%s, message:%s", message->detail, message->message)));
		}
	}
	return (message == NULL);
}
